...
pipeline {
    ...
    parameters {
        string(name: 'APP_VERSION', defaultValue: '', description: 'Immutable version to deploy, e.g. 7.9. Required.')
    }
    ...
    environment {
        ...
        // Secret is bound to a file at runtime and never echoed (Phase 7).
    ...
    options {
        ...
        // uncontrolled duplicate candidate containers (Phase 7).
    ...
    stages {
        ...
        stage('Validate Version') {
            steps {
                ...
                sh "scripts/validate-version.sh '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}'"
            }
        }
        ...
        stage('Docker Build') {
            steps {
                script {
                    env.IMAGE_TAG = sh(
                        script: "scripts/build-image.sh '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}' | tail -n1",
                        returnStdout: true
                    ).trim()
                }
        ...
        stage('Docker Image Validation') {
            steps {
                sh "scripts/validate-image.sh '${env.IMAGE_TAG}' '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}'"
            }
        }
        ...
        stage('Start Candidate') {
            steps {
                ...
                //   withCredentials([file(credentialsId: 'orders-api-secret-env', variable: 'SECRET_ENV_FILE')]) { ... }
                // so its contents never appear in the Jenkinsfile, SCM, or
                // console log (Phase 7: no secrets in logs). This repo ships
                // scripts/secrets/orders-api.env.example as the local stand-in.
                script {
                    def out = sh(
                        script: "scripts/start-candidate.sh '${env.IMAGE_TAG}' '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}'",
                        returnStdout: true
                    ).trim()
                    echo out
                    out.split('\n').each { line ->
        ...
        stage('Container Validation') {
            steps {
                sh "scripts/container-validation.sh '${env.CANDIDATE_NAME}' '${env.CANDIDATE_PORT}'"
            }
        }

        stage('Application Health Check') {
            steps {
                sh "scripts/health-check.sh '${env.CANDIDATE_PORT}' '${env.CANDIDATE_NAME}' 10 2"
            }
        }

        stage('Integration Check') {
            steps {
                sh "scripts/integration-check.sh '${env.CANDIDATE_PORT}' '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}'"
            }
        }

        stage('Traffic Switch') {
            steps {
                sh "scripts/switch-traffic.sh '${env.CANDIDATE_COLOR}' '${params.APP_VERSION}'"
            }
        }

        stage('Old Version Cleanup') {
            steps {
                sh "scripts/cleanup-old.sh '${env.CURRENT_COLOR}'"
                sh "KEEP_LAST_N=3 scripts/cleanup-images.sh"
            }
        }

        stage('Deployment Verification') {
            steps {
                sh "scripts/deployment-verification.sh '${params.APP_VERSION}' '${env.GIT_COMMIT_SHA}' '${env.CANDIDATE_COLOR}'"
                archiveArtifacts artifacts: ".evidence/deployment-${params.APP_VERSION}.json", allowEmptyArchive: false
            }
        }
    }
    ...
    post {
        failure {
            script {
                ...
                if (env.CANDIDATE_COLOR && env.CURRENT_COLOR) {
                    sh "scripts/rollback-candidate.sh '${env.CANDIDATE_COLOR}' '${env.CURRENT_COLOR}' || true"
                } else {
        ...
        always {
            sh "docker ps --filter name=orders- --format 'table {{.Names}}\\t{{.Image}}\\t{{.Status}}\\t{{.Ports}}' || true"
        }
    }
}
