pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    if (!params.APP_VERSION?.trim()) {
                        error('APP_VERSION parameter is required, e.g. APP_VERSION=7.9')
                    }
                }
            ...
            steps {
                script {
                    ...
                    if (!env.IMAGE_TAG.startsWith("${env.IMAGE_NAME}:")) {
                        error("Invalid image tag returned by build-image.sh: ${env.IMAGE_TAG}")
                    }
        ...
        stage('Start Candidate') {
            steps {
                withCredentials([
                    string(credentialsId: 'orders-api-secret', variable: 'ORDERS_API_SECRET_VALUE')
                ]) {
                    script {
                        try {
                            sh '''
                                ...
                                chmod 600 "$WORKSPACE/scripts/secrets/orders-api.env"
                            ...
                            if (!out) {
                                error('start-candidate.sh returned no output.')
                            }
                            ...
                            if (!env.CURRENT_COLOR) {
                                error('start-candidate.sh did not return CURRENT_COLOR.')
                            }
                            if (!env.CANDIDATE_COLOR) {
                                error('start-candidate.sh did not return CANDIDATE_COLOR.')
                            }
                            if (!env.CANDIDATE_NAME) {
                                error('start-candidate.sh did not return CANDIDATE_NAME.')
                            }
                            if (!env.CANDIDATE_PORT) {
                                error('start-candidate.sh did not return CANDIDATE_PORT.')
                            }
                        } finally {
                            sh '''
                                rm -f "$WORKSPACE/scripts/secrets/orders-api.env" || true
                            '''
        ...
        failure {
            script {
                ...
                if (env.CANDIDATE_COLOR && env.CURRENT_COLOR) {
                    ...
                    echo "Inspecting actual router state and performing safe deployment rollback..."
                    ...
                    echo "Rollback skipped. Required deployment color variables are unavailable."
                }
            }
